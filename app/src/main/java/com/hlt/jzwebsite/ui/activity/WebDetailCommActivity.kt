...
import kotlinx.android.synthetic.main.activity_web_detail_common.*
 ...
 * @date :2020/3/6 17:48
 */
class WebDetailCommActivity : BaseActivity() {
    ...
    override var layoutId: Int = R.layout.activity_web_detail_common
    ...
    override fun initData() {
        ...
        tv_title.apply {
            visibility = View.VISIBLE
            text = getString(R.string.title_empty)
            isSelected = true
        }
        ...
        initWebView()
        if (title_detail.equals("联系方式") || title_detail.equals("企业简章")
            || title_detail.equals("为民服务体系") || title_detail.equals("跨融直播")
        ) {
            time.visibility = View.GONE
            subtitle.visibility = View.GONE
            cons_bottom.visibility = View.GONE
            tv_title.text = title_detail
        } else {
        ...
        viewModel.run {
            handleData(detailData) {
                ...
                if (webDetailResult.image.isNullOrEmpty()) {// 空设置默认图
                    iv_topBg.visibility = View.GONE
                } else {
                    iv_topBg.visibility = View.VISIBLE
    ...
    private fun toCollect(status: Int) {
        ...
        modifyPhoneViewModel.run {
            handleData(collect) {
                Logger.d("收藏成功---->>>")
                iv_collect.setBackgroundResource(com.hlt.jzwebsite.R.mipmap.ic_collected)
                iv_collect.invalidate()
                tv_collect.setText("已收藏")
                ToastUtils.show(getString(com.hlt.jzwebsite.R.string.collect_succ))
                //刷新收藏标识
                statusF = 1
...
//            web_view.loadUrl(webUrl)
    ...
    private val mWebViewClient = object : com.just.agentweb.WebViewClient() {
        ...
        override fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
            ...
            Log.d("mWebViewClient", "shouldOverrideUrlLoading->$url")
            if (url?.endsWith(".apk")!!) {
                val uri = Uri.parse(url)
                val viewIntent = Intent(Intent.ACTION_VIEW, uri)
                this@WebDetailCommActivity.startActivity(viewIntent)
                return true
            }
